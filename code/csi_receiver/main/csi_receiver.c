...
unsigned next_mrc_pca_timestamp = 30 * SECONDS_TO_MICROSECONDS; // start after 30 seconds to give the ESPs some time to start sending/receiving
...
static void csi_data_print_task(void *arg)
{
    ...
    while (xQueueReceive(g_csi_info_queue, &info, portMAX_DELAY)) {
        ...
        if(timestamp_array[current_last_element]>next_mrc_pca_timestamp){
            ...
            next_mrc_pca_timestamp += MRC_PCA_EVERY_X_SECONDS * SECONDS_TO_MICROSECONDS;
